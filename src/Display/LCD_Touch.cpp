...
#include "Arduino.h"
#else
#include "LCD_Driver_SDL.h" // For mock functions
#include <cmath>
#include <cstdio>
#include <cstdlib>
#include <cstring>
...
*******************************************************************************/
POINT TP_Read_ADC(unsigned char CMD) {
  POINT Data = 0;
...
the average parameter: Channel_Cmd :   0x90 :Read channel Y + 0xd0 :Read channel
x +
*******************************************************************************/
#define READ_TIMES 5 // Number of readings
#define LOST_NUM 1   // Discard value
static POINT TP_Read_ADC_Average(unsigned char Channel_Cmd) {
  unsigned char i, j;
  POINT Read_Buff[READ_TIMES];
  POINT Read_Sum = 0, Read_Temp = 0;
...
/*******************************************************************************
  ...
  parameter:
    Channel_Cmd :   0x90 :Read channel Y +
                    0xd0 :Read channel x +
*******************************************************************************/
static void TP_Read_ADC_XY(POINT *pXCh_Adc, POINT *pYCh_Adc) {
...
static bool TP_Read_TwiceADC(POINT *pXCh_Adc, POINT *pYCh_Adc) {
  POINT XCh_Adc1, YCh_Adc1, XCh_Adc2, YCh_Adc2;
  ...
  if (!GET_TP_IRQ) { // Press the button to press
    ...
    if (chCoordType) {
      ...
      if (sTP_DEV.TP_Scan_Dir ==
        ...
        sTP_Draw.Xpoint = sTP_DEV.fXfac * sTP_DEV.Ypoint + sTP_DEV.iXoff;
        sTP_Draw.Ypoint = sTP_DEV.fYfac * sTP_DEV.Xpoint + sTP_DEV.iYoff;
      } else {
        sTP_Draw.Xpoint = sLCD_DIS.LCD_Dis_Column -
...
unsigned char TP_Scan(unsigned char chCoordType) {
  ...
  int x, y;
  bool pressed;
  SDL_GetMouseState_Mock(&x, &y, &pressed);

  if (pressed) {
    // If pressed, update Xpoint/Ypoint
    // Note: The hardware driver does calibration here.
    // For SDL, we get exact screen coordinates, so we can map them directly
    // to sTP_Draw.Xpoint (which are the screen coords).
    // sTP_DEV.Xpoint are usually ADC values.

    // For simplicity, we assume perfect calibration for PC
    sTP_Draw.Xpoint = x;
    sTP_Draw.Ypoint = y;

    // Also update sTP_DEV.Xpoint/Ypoint just in case, though they are ADC
    // values in real HW
    sTP_DEV.Xpoint = x;
    sTP_DEV.Ypoint = y;

    if (0 == (sTP_DEV.chStatus & TP_PRESS_DOWN)) { // Not being pressed before
      sTP_DEV.chStatus = TP_PRESS_DOWN | TP_PRESSED;
      sTP_DEV.Xpoint0 = x;
      sTP_DEV.Ypoint0 = y;
    }
    ...
    if (sTP_DEV.chStatus & TP_PRESS_DOWN) { // 0x80
      ...
      sTP_DEV.Xpoint0 = 0;
      sTP_DEV.Ypoint0 = 0;
      // sTP_DEV.Xpoint = 0xffff; // Keep last known or reset?
      // sTP_DEV.Ypoint = 0xffff;
    }
  }
  return (sTP_DEV.chStatus & TP_PRESS_DOWN);
...
// Stubs for internal functions that are not needed for PC but referenced by
// TP_Adjust
POINT TP_Read_ADC(unsigned char CMD) { return 0; }

static void TP_Read_ADC_XY(POINT *pXCh_Adc, POINT *pYCh_Adc) {
  // For PC, just return mouse coordinates as "ADC" values
  int x, y;
  SDL_GetMouseState_Mock(&x, &y, NULL);
  *pXCh_Adc = x;
  *pYCh_Adc = y;
}

static bool TP_Read_TwiceADC(POINT *pXCh_Adc, POINT *pYCh_Adc) {
  TP_Read_ADC_XY(pXCh_Adc, pYCh_Adc);
  return true;
}
#endif
    ...
    if ((sTP_DEV.chStatus & 0xC0) == TP_PRESSED) {
      ...
      default:
        ...
        TP_DrawCross(sLCD_DIS.LCD_Dis_Column - Mar_Val,
                     sLCD_DIS.LCD_Dis_Page - Mar_Val, WHITE);
        TP_DrawCross(Mar_Val, Mar_Val, RED);
        GUI_DisString_EN(40, 26, "TP Need readjust!", &Font16, FONT_BACKGROUND,
                         RED);
        break;
      }
    }
